...
const CreateGame: React.FC = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('room-created', ({ roomId, gameState }: { roomId: string; gameState: GameState }) => {
  ...
  const handleMove = (x: number, y: number) => {
    if (!socket || !gameState || waiting) return;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-700">Creating game room...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2">
              <Board
                ...
                onMove={handleMove}
                disabled={waiting || gameState.currentPlayer !== playerColor || !!gameState.winner}
              />
            </div>
